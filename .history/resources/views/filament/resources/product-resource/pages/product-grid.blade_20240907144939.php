        ...
        @foreach ($products as $product)
            <div class="border rounded-lg p-4  shadow">
                ...
                <form action="{{ route('cart.add', $product->id) }}" method="POST">
                    ...
                    <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded mt-2 dark:bg-blue-700 dark:text-gray-200">
                        Add to Cart
                    </button>
                </form>
                ...
                <form action="{{ route('cart.buy', $product->id) }}" method="POST">
                    @csrf
                    <input type="hidden" name="quantity" value="1">
                    <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded mt-2 dark:bg-green-700 dark:text-gray-200">
                        Buy Now
                    </button>
                </form>
