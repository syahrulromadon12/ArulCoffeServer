        ...
        @foreach ($products as $product)
            <div class="border rounded-lg p-4 bg-gray-800 shadow">
                ...
                <form action="{{ route('cart.add', $product->id) }}" method="POST">
                    ...
                    <button type="submit" class="bg-blue-500 text-black dark:text-white px-4 py-2 rounded mt-2 hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        Add to Cart
                    </button>
                </form>
                
                <form action="{{ route('check', $product->id) }}" method="POST">
                    @csrf
                    <input type="hidden" name="quantity" value="1">
                    <button type="submit" class="bg-green-500 text-black dark:text-white px-4 py-2 rounded mt-2 hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500">
                        Buy Now
                    </button>
                </form>
